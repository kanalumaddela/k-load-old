...
if (empty($_SESSION['steam_uptodate']) or empty($_SESSION['steam_personaname']) || !empty($steam64)) {
    ...
    if (!empty($content['response']['players'][0]['commentpermission'])) {
        $_SESSION['steam_commentpermission'] = $content['response']['players'][0]['commentpermission'];
    }
    if (!empty($content['response']['players'][0]['gameextrainfo'])) {
        $_SESSION['steam_gameextrainfo'] = $content['response']['players'][0]['gameextrainfo'];
    }
    if (!empty($content['response']['players'][0]['gameserverip'])) {
        $_SESSION['steam_gameserverip'] = $content['response']['players'][0]['gameserverip'];
    }
    if (!empty($content['response']['players'][0]['loccountrycode'])) {
        $_SESSION['steam_loccountrycode'] = $content['response']['players'][0]['loccountrycode'];
    }
    if (!empty($content['response']['players'][0]['locstatecode'])) {
