...
body {
    ...
    <?php
    if (isset($user_bg_type)) {
        $background['type'] = $user_bg_type;
    }
    if ($background['type'] >= 2 && !empty($background['image'])) {
        echo 'background-image:url('.$background['image'].') !important;'."\n";
    }
    ?>
}
...
<?php if (file_exists('themes/'.$_GET['theme'].'/'.$_GET['theme'].'.css')) {
        include 'themes/'.$_GET['theme'].'/'.$_GET['theme'].'.css';
    } ?>
